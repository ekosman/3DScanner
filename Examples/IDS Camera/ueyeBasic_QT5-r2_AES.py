...
class CaptureVideoWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.aes_button = QPushButton("AES: start/stop")
        ...
        layout.addWidget(self.aes_button, alignment=Qt.AlignTop)
        layout.addWidget(self.graphics_view)
        self.setLayout(layout)
        self.start_stop_button.clicked.connect(self.switch_capturing)
        self.aes_button.clicked.connect(self.auto_shutter)
    ...
    def init(self):
        ...
        if ret != ueye.IS_SUCCESS:
            ...
            result = QMessageBox.critical(
                error_message, "Error", "Invalid device id!", buttons=QMessageBox.Ok
            )
    ...
    def convert_image_data(self):
        ...
        ueye.is_AOI(
            self.hCam, ueye.IS_AOI_IMAGE_GET_AOI, rect_aoi, ueye.sizeof(rect_aoi)
        )
        ...
        while True:
            ...
            if ret == 0:
                ...
                self.image_data_copy = (
                    ueye.CHAR * int(self.img_data.width * self.img_data.height * 3)
                )()
                ...
                self.image_data_copy = numpy.reshape(
                    self.image_data_copy,
                    (
                        int(self.img_data.height),
                        int(self.img_data.width),
                        bytes_per_pixel,
                    ),
                )
    ...
    def auto_shutter(self):
        # https://en.ids-imaging.com/manuals/uEye_SDK/EN/uEye_Manual_4.91.1/is_setautoparameter.html
        if self.autogain:
            param = ueye.DOUBLE(0)
            param2 = ueye.DOUBLE(0)
            ueye.is_SetAutoParameter(
                self.hCam, ueye.IS_SET_ENABLE_AUTO_SHUTTER, param, param2
            )
            self.autogain = False
        else:
            param = ueye.DOUBLE(64)
            param2 = ueye.DOUBLE(0)
            nret = ueye.is_SetAutoParameter(
                self.hCam, ueye.IS_SET_AUTO_REFERENCE, param, param2
            )
            print(nret)
            param = ueye.DOUBLE(1)
            nret = ueye.is_SetAutoParameter(
