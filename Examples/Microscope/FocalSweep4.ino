/*
  ...
  The Arduino controls 3 system components - the camera, LED's and optotune electronically tunable lens (ETL).
  System component: Physical parameter which is controlled --> System parameters which can be adjusted
    1. Camera: Trigger Camera Exposure --> Frames Per Second (Computer controls: turn saving on/off, exposure length)
      cl = live view (cl1 = on, cl0 = off)
      cs = single focal scan
      cp = single photometric circle
      ct = start time series
      ct# = change time series interval length (min)
      cf# = change frames per ??

    2. Optotune: Current Supplied to Optotune --> Number of Focal Steps, Current Focal Step
      of = step one focal step forward
      ob = step one focal step backward
      oc# = choose specific focal step number
      os# = change total number of focal steps

  SPI Interface - Control
    The circuit: CS - to digital pin 0  (SS pin), SDI - to digital pin 2 (MOSI pin), CLK - to digital pin 1 (SCK pin)
*/
...
const int all = 0;const int w = 1; const int b = 2; const int r = 3; const int e = 4; const int off = 5;// duration of led pulses (microseconds)
int ledMode = all;
...
int extraSteps;//extra exposure after stack is finished (to ensure program does not get stuck)
...
float wd; float wdMax = 86; float wdMin = 60; // bfl = back focal length (mm)
float optotuneAmp;  // optotuneAmp = -0.0001242*bfl^3 + 0.06884*bfl^2 - 13.83*bfl + 1015.2 (in milli-amps)
float driverAmp_max = 300; // in milli-amps
float driverAmp_max_use = 250;
int optotuneAmp_list[200];
int driverLevel; // driverLevel is 12 bit = 0 - 4095, driverLevel = (optotuneAmp / driverAmp_max) * 4095
int driverVals[200]; //values found from optotune manual, set driverVals array with 100 slots
int wdVals[200];
...
int driverByte1; int driverByte2; //data sent to driver: 4 bit address, 12 bit current level -> split into 2 bytes (8 bits each)
//Reading Input
int incomingByte;
String command;
int setting = 0;
...
void setup()
{
  ...
  SPI.setBitOrder(MSBFIRST); SPI.setDataMode(SPI_MODE0); SPI.setClockDivider(SPI_CLOCK_DIV4);
  //Camera + Projector
  pinMode(cameraTrigger, OUTPUT);
  pinMode(externalTrigger, OUTPUT); // projector
  pinMode(ledTrigger, OUTPUT); //LED
  Serial.flush(); //make sure nothing's in the buffer at the beginning of the run
}
...
void loop()
{
  ...
  if (firstLoop == 1)
  {
    Serial.println("Hello Teensy Started, Waiting for order..."); //***Making Read Input Not Work, Very Odd***
    setting = 'q'; mode = 'S'; value = stepTotal; optotuneControl(); firstLoop = 0;
  }
  if (Serial.available())
  {
    readInput();
  ...
  if (setting == 'P' || setting == 'p') {
    setParameters(); setting = 'q';
  }
  //Imaging
  if (setting == 'C' || setting == 'c') // Capture
  {
    if (mode == 'S' || mode == 's') {
      captureFocalSweep(); setting = 'q';//Single focal sweep
    }
    if (mode == 'R' || mode == 'r'){
      advanceCamera(); setting = 'q';//Advance projector
    }
    if (mode == 'm') {
      captureSingleStep(); setting = 'q';//Single focal sweep in slow motion
    }
    if (mode == 'l') {
...
void readInput()
{
  ...
  //Serial.println(""); Serial.print("Input: "); Serial.print(command); Serial.println(); //display complete input
...
{
  digitalWrite(slaveSelectPin, LOW); // take the SS pin low to initialize SPI communication
  driverByte1 = (addressSPI << 4) + ( (unsigned int)driverLevel >> 8 ); //SPI address shifted 4 bits, first 4 bits of level
  driverByte2 = 255 & driverLevel; //last 8 bis of level (calculated using bitwise logic, 255 = 11111111)
  SPI.transfer(driverByte1); SPI.transfer(driverByte2);   //  send in the address and value via SPI
...
void optotuneControl()
{
  ...
  {
    stepTotal = value; Serial.print("New Number of Steps: "); Serial.println(stepTotal);
    for (i = 0; i < stepTotal; i++)
    {
      optotuneAmp =  ((float)i + 0.5) / (float)stepTotal * driverAmp_max_use ;
      //wd = (float)wdMax - i * ( (wdMax-wdMin) / ((float)stepTotal-1) ); // (BFL = 210mm to 80mm range)
      //optotuneAmp =  -0.00025058632*pow(wd,4) + 0.07517730392*pow(wd,3) - 8.49438801629*pow(wd,2) + 419.36852014456*wd - 7344.10899548123;

      optotuneAmp_list[i] = optotuneAmp * 1;
      driverVals[i] = (optotuneAmp / driverAmp_max) * 4095;
      wdVals[i] = 0;
      Serial.print("Step:"); Serial.print(i); Serial.print("  WD:"); Serial.print(wd); Serial.print("mm  Current:"); Serial.print(optotuneAmp); Serial.print("mA  DriverVal: "); Serial.println(driverVals[i]);
      //Serial.println(optotuneAmp);
    }
     Serial.print("Current Step: "); Serial.print(stepNum); Serial.print(" ( "); Serial.print(stepTotal); Serial.print(" total steps)");
    Serial.print("  Current:"); Serial.print(optotuneAmp_list[stepNum]); Serial.println("mA");

  int stepExposure       =  exposureTime / stepTotal ;
  int prePatternDelay  =  int( ((10000.0-float(projExposurePer)) * float(stepExposure)) / 10000.0);  // ursprunglich at 100.0
  ...
  Serial.print("Delay: ");             Serial.print(prePatternDelay);  Serial.println("us");
...
///////////////////
void cameraDelay() { {delay(1000 / fps - (long)ledTime[ledMode]/1000);} }
/////////////////////////////////
...
void setParameters()
{
  ...
  {
    exposureTime = value; Serial.print("Exposure: "); Serial.println(exposureTime);
  }

  if (mode == 'p')  // Change projector exposure
  {
    projExposurePer = value; Serial.print("Projector exposure: "); Serial.println(projExposurePer);
  }

  if (mode == 'l')  // Change use_LED status
  {
    use_LED = value; Serial.print("Use LED: "); Serial.println(use_LED);
  }

  if (mode == 'c')  // Change max_current
  {
    driverAmp_max_use = float(value); Serial.print("Max current use: "); Serial.println(driverAmp_max_use);
  }
...
void FocalStack()
{
  ...
  for (int extraSteps = 0; extraSteps < 1; extraSteps++) //extra exposure after stack is finished (to ensure program does not get stuck)
  {
    cameraDelay();cameraExposure(); //extra exposure after stack is finished
  }
}
...
void captureFocalSweep()
{
  ...
  for (int i = 0; i < stepTotal; i++)
  {
    ...
    //currentReport = ((float)driverVals[i] / 4095) * driverAmp_max; Serial.print(" ("); Serial.print(currentReport); Serial.print("mA)"); Serial.print(", ");
